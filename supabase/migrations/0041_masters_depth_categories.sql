-- Adds a shallow depth chart for Masters — 1st-3rd for 8+ and 4+ only (the
-- two boat classes masters actually fields) — on top of the existing flat
-- 'masters' category, which is kept for backward compatibility with data
-- written before this split. Also makes these selectable as a Fleet boat
-- type, which masters previously had no boat_category for at all.
