...
export function ImportItemsForm({ eventId }: { eventId: string }) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const rows: FoodTentItemImportRow[] = rawRows.map((raw) => {
        const normalized: Record<string, string> = {};
        for (const [key, value] of Object.entries(raw)) {
          normalized[normalizeKey(key)] = String(value ?? "").trim();
        }
        return {
          title: normalized.title ?? normalized.item ?? normalized.name,
          quantity_needed: normalized.quantity_needed ?? normalized.quantity ?? normalized.qty,
  ...
  return (
    <div className="border rounded-lg p-4 flex flex-col gap-3 max-w-md">
      <div className="flex items-center justify-between">
        <h2 className="font-medium">Import items from Excel</h2>
        <button
          type="button"
      ...
      <p className="text-sm text-gray-500">
        Columns expected (case-insensitive): Title (or Item/Name), Quantity
        Needed (or Quantity/Qty), Notes. Only Title is required — quantity
        defaults to 1. Each row adds a new item request for this event.
      </p>
      ...
      {result && (
        <div className="text-sm">
          <p className="text-green-700">Added {result.imported} item(s).</p>
